...
public class Inventory : MonoBehaviour
{
    ...
    [Header("物品冷却")]
    private float lastTimeUesFlask;
    ...
    private void UpdateFlaskUI()
    {
        ...
        foreach (var slot in equipmentSlot)
        {
            ...
            {
                bool canUse = CanUseFlask();
                slot.UpdateFlaskState(canUse);
            }
        }
    }
    public void UesFlask()
    {
        ...
        //  获取对应 InventoryItem
        InventoryItem item = equipmentDictionary[currentFlask];
    ...
    public bool CanUseFlask()
    {
        ...
        if (player.GetComponent<PlayerStats>().currentHealth >=
            player.GetComponent<PlayerStats>().GetMaxHealthValue())
            return false;

        //  冷却中
        if (Time.time <= lastTimeUesFlask + currentFlask.itemCooldown)
            return false;
